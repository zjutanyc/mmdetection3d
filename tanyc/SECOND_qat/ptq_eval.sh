...
DEFAULT_CKPT="work_dirs/SECOND_qat/sparse_encoder_ptq_calib1200.pth"
...
# Embrace randomness: set hash seed to a random value and allow CuDNN autotune.
...
python "${REPO_DIR}/tanyc/SECOND_qat/CUDA-SECOND/second_ptq_eval.py" \
    "$CONFIG" \
    --checkpoint "$CHECKPOINT" \
    "$@"
    "$CONFIG" \
    --checkpoint "$CHECKPOINT" \
    --calib-batches "${CALIB_BATCHES:-1200}" \
    --save-renamed \
    "$@"
